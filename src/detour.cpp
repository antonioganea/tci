...
void CreateDetour()
{
    ...
    //0x2C313A <- DayZ 1.28.161464
    //Moving onwards, the pattern scanning is automatic

    uint64_t moduleBase = (uint64_t)GetModuleHandle(NULL);

    INJECTION_POINT = Pattern::PatternScan(GetModuleHandle(NULL),
    "49 8b 8e b8 01 00 00 4c 8d ?? ?? ?? ?? ?? 48 39 01 0f 84 ?? ?? ?? ?? 48 89 01 e9 ?? ?? ?? ?? 8b 47 38 83 f8 01 0f 85 ?? ?? ?? ?? 44 8b c0");

    if (INJECTION_POINT == 0)
    {
        console.AddLog("Couldn't find injection point for detour (signature scanning failed)\n");
        return;
    }

    console.AddLog("Found injection point for detour through signature scanning at %p\n", (INJECTION_POINT-moduleBase));

    console.AddLog("Creating hook for detour ..!\n");
    enfusion_script_vm_hook = safetyhook::create_mid(INJECTION_POINT, [] (safetyhook::Context& ctx)
    { 
        // console.AddLog("Hello from genre text mid-function hook.\n");
        OnGetControl();
    });

    console.AddLog("Detour Hook activated ..!\n");
...
///////// OLD IMPLEMENTATION (manually hooking)
...
void CreateDetour_OLD() {
    ...
    const uint64_t relativeAddrFuncA = 0x2C313A;
