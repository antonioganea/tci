...
void goInLua() {
    ...
    {
        std::unique_lock<std::mutex> lk(m);
        waitStage = CV_WAIT_STAGE::GO_IN_LUA;
        cv.wait(lk, [] {return (inLua == false); }); // it happened before to block here (debug mode exception)
    }
}
...
void goOutOfLua() {
    ...
    {
        std::unique_lock<std::mutex> lk(m);
        waitStage = CV_WAIT_STAGE::GO_OUT_OF_LUA;
        cv.wait(lk, [] {return (inLua == true); }); // runtime gui debug info shows it jamming here
    }
}
...
std::thread* setup(const char* path) {
    ...
    *DLL_COMMAND = (int)DayZServerCommands::JustBooted;
    ...
    {
        std::unique_lock<std::mutex> lk(m);
        waitStage = CV_WAIT_STAGE::THREAD_SETUP;
