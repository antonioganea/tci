...
void gui::DestroyHWindow()
{
	...
	UnregisterClass((LPCSTR)windowClass.lpszClassName, windowClass.hInstance); // Not sure if the (LPCWSTR) cast is okay.
}
...
extern CV_WAIT_STAGE waitStage;
...
void    TCIAppConsole::Draw(const char* title, bool* p_open)
{
    ...
    /*
    ImGui::SameLine();
    ImGui::Checkbox("inLua", &inLua);
    ImGui::SameLine();
    ImGui::Checkbox("isLuaPowered", &isLuaPowered);

    if (ImGui::Button("Notify CV"))
    {
        cv.notify_one();
    }
    ...
    switch(waitStage)
    {
        case CV_WAIT_STAGE::GO_IN_LUA:
            ImGui::LabelText("waiting", "GO_IN_LUA");
            break;
        case CV_WAIT_STAGE::GO_OUT_OF_LUA:
            ImGui::LabelText("waiting", "GO_OUT_OF_LUA");
            break;
        case CV_WAIT_STAGE::THREAD_SETUP:
            ImGui::LabelText("waiting", "THREAD_SETUP");
            break;
    }
    */
